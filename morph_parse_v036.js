...
function linearize(parses) {
	...
	//helper function -- recursivey un-nest -- sort into prefixes / suffixes
	function iter(parse, prefixes, suffixes) {
		...
		if (keys.includes("morpheme_type")) {
			...
			if (parse.morpheme_type == "PFX" || parse.morpheme_type == "CFXPFX") {
				...
				suffixes = suffixes.concat([morpheme]);
			} else {   //Words and non-affixing-words
				let rev_sfx = suffixes;   //copy, because .reverse() changes the original
				linear_parses.push(prefixes.concat([morpheme].concat(rev_sfx.reverse())));
				//rev_sfx.reverse();  //reverse it back -- weird carryover bug??
			}
		}
