...
window.onload = function(){
	...
	// (hover preview popup = 200px x 300px)
	if (results_div) {
		results_div.addEventListener('mouseover', (event) => {
			if (event.target.closest('.internal-link')) {
				const e = event.target.closest('.internal-link');
				const rect = e.getBoundingClientRect();	// link position on page
				popup_timer = setTimeout(() => {
					//position the popup div
					if (rect.left > (window.innerWidth / 2)) {	// if link is in right half of screen
						temp_left = rect.right - 300;	// position right edge of popup at right edge of link
						temp_left = (temp_left < 0) ? temp_left = "5px" : temp_left += "px";	// if it's off-screen now, move back on (+5px padding)
						hover_preview_popup.style.left = temp_left;
					} else {	// if link is in left half
						var body_r = document.body.getBoundingClientRect().right;
						temp_left = rect.left;	// position left edge of popup at left edge of link
						temp_left = (temp_left + 300 > body_r) ? body_r - 305 + window.scrollX + "px" : temp_left += "px";	// if offscreen move back (+5p padding)
						hover_preview_popup.style.left = temp_left;
					}
