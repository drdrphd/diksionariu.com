...
//
function displayCHResults(str, results, message = "", all_words = [], type = "") {
	...
	if (results == "0") {
		...
		//Otherwise, run fuzzy search
			...
			formatted_results = 
				"<h2>" + str + "</h2>" +
				"<p class='no-result'>Entry not found</p></br>" +
				"<h3>Did you mean: </h3>" + fuzzySearch(str, all_words);
		...
		//For Direct fuzzy search (returns type: "fuzzy")
			...
			formatted_results += "<h2>Near matches: <i>"
								+ str
								+ "</i></h2>"
								+ fuzzySearch(str, all_words, true);
		}
	}
	...
	if (results != "0") {
		...
		if (str.includes("::")) {
			...
			for (var i = 0; i < results.length; i++)   results[i].examples   = addHighlights(results[i].examples, str);
		}
...
function displayCHAltResults(str, results) {
	...
	if (alt_hits == 1) {
		var message = "<zz class='redirect-text'>(Redirected from <a href='" + str + "'>" + str + ")</a></zz>";
		displayCHResults(results[last_i].entry, [results[last_i]], message);
		return;
	}
	...
	//highlight the search terms in the results
	for (var i = 0; i < results.length; i++) {
		results[i].alternate_forms = addHighlights(results[i].alternate_forms, str);
...
// Gets the ~20 surrounding entries (by index) (might allow this nuber to change via global setting)
